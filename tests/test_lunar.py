...
def test_oracle_fixture_expectations_for_fail_ok_good_scripts(script_name: str, expected_verdict: str) -> None:
    script_paths = {
        "test/fail.asc": FAIL_SCRIPT,
        "test/ok.asc": OK_SCRIPT,
        "test/good.asc": GOOD_SCRIPT,
    }
    burns = _read_burn_script(script_paths[script_name])

    if script_name == "test/fail.asc":
        assert burns[:8] == [0, 0, 0, 0, 0, 0, 0, 170]
        assert burns[-4:] == [0, 0, 0, 20]
        assert max(burns) == 200
    elif script_name == "test/ok.asc":
        assert burns[:8] == [0, 0, 0, 0, 0, 0, 0, 170]
        assert burns[-4:] == [8, 10, 9, 100]
        assert burns.count(200) == 6
    else:  # test/good.asc
        assert burns[:5] == [0, 0, 0, 0, 0]
        assert burns[-5:] == [100, 70, 39, 21, 15]
        assert min(burns) == 0
